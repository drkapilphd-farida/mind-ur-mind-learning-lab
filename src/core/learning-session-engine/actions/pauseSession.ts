...
// Universal Learning Session Engine™ (LSE-1). Pauses an active session
// — real queue/position/progress are left untouched, only status
// changes and a real session-paused event is appended.
export function pauseSession(session: LearningSession, options: SessionActionOptions = {}): SessionActionResult {
  const transition = validateTransition('pause', session.status)
  ...
  const events = [{ id: idFactory(), type: 'session-paused' as const, occurredAt: nowIso }]
