// Best-stats store for Image Flash Grid™ — a small, narrowly scoped
// parallel to every other exercise's own local-history file in this
// project (wordFlashGridLocalHistory.ts, numberFlashGridLocalHistory.ts),
...
// WordFlashGridBestStats/NumberFlashGridBestStats exactly
...
export type ImageFlashGridBestStats = {
  bestScorePercent: number
  bestStreak: number
}

const EMPTY_BEST: ImageFlashGridBestStats = { bestScorePercent: 0, bestStreak: 0 }

export function loadBestImageFlashGridStats(storageKey: string): ImageFlashGridBestStats {
...
export function recordBestImageFlashGridStats(storageKey: string, session: ImageFlashGridBestStats): ImageFlashGridBestStats {
  const previous = loadBestImageFlashGridStats(storageKey)
  const next: ImageFlashGridBestStats = {
