...
import { ROUNDS_PER_SESSION, PERFECT_SESSION_BONUS } from '../quantumMentalRotationDataset'

type QuantumMentalRotationCompleteScreenProps = {
  elapsedMs: number
  correctCount: number
  totalScore: number
  bestStreak: number
...
// are hard WPM-shaped and don't describe a mental-rotation sprint
...
export function QuantumMentalRotationCompleteScreen({
  elapsedMs,
  correctCount,
  totalScore,
  bestStreak,
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center gap-10 px-6 py-16 text-center">
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Rotation Complete</h1>
        <p className="mt-2 text-sm text-muted-foreground">Nice spatial reasoning.</p>
        {isPerfectSession && (
          <p className="mt-2 text-sm font-semibold text-emerald-600">Flawless rotation! +{PERFECT_SESSION_BONUS} bonus included.</p>
        )}
      </div>

      <div className="grid w-full grid-cols-2 gap-4">
        <ReadingStatTile variant="card" label="Rotation Accuracy" value={`${accuracyPercent}%`} />
