...
import { ROUNDS_PER_SESSION, PERFECT_SESSION_BONUS } from '../afterImageGazingDataset'

type AfterImageGazingCompleteScreenProps = {
  elapsedMs: number
  clearCount: number
  totalScore: number
  bestStreak: number
  bestClarityPercentAllTime: number
...
// are hard WPM-shaped and don't describe a gazing session honestly.
...
// the exact same visual language/classes as every sibling gamified
// exercise's own completion screen, without modifying or forking the
// locked component itself. "Clarity" is deliberately named to avoid
// implying "correctness" — it's the % of rounds a learner honestly
// self-reported as a clear afterimage, nothing more.
export function AfterImageGazingCompleteScreen({
  elapsedMs,
  clearCount,
  totalScore,
  bestStreak,
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center gap-10 px-6 py-16 text-center">
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Gazing Complete</h1>
        <p className="mt-2 text-sm text-muted-foreground">Great focus training.</p>
        {isPerfectSession && (
          <p className="mt-2 text-sm font-semibold text-emerald-600">Clear every round! +{PERFECT_SESSION_BONUS} bonus included.</p>
        )}
      </div>

      <div className="grid w-full grid-cols-2 gap-4">
        <ReadingStatTile variant="card" label="Clarity Rate" value={`${clarityPercent}%`} />
        <ReadingStatTile variant="card" label="Clear Rounds" value={`${clearCount} / ${ROUNDS_PER_SESSION}`} />
        ...
        <ReadingStatTile variant="card" label="Best Clarity (All-Time)" value={`${bestClarityPercentAllTime}%`} />
