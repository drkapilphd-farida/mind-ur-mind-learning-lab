...
// Top-level orchestrator for Sentence Reading Mode™ (10/10 Overhaul) — the
// Master Reading Engine's third mode. Fully replaces the previous
// block-gated architecture (3 fixed Levels, each ending in a pass/retry
...
// zero mid-stream pop-ups. Structurally mirrors
// VerticalChunkSlidingExperience.tsx (same engine, same session pipeline,
// same local-history pattern, same single ungated post-reading quiz gate),
// plus an orientation choice (horizontal/vertical) that picks which
// Canvas renders — both feed the engine the exact same units/pacing. A
...
export function SentenceReadingModeExperience({ onComplete }: SentenceReadingModeExperienceProps = {}): React.JSX.Element {
  ...
  const [orientation, setOrientation] = useState<SentenceFlowOrientation>('horizontal')
  // Comprehension quiz gate — see SentenceReadingModeQuiz.tsx's own doc
  ...
  // needs that value, only the picked category's own questions). Zero
  // Mid-Stream Interruptions — this is the ONLY quiz moment in the entire
  // session, always after the full passage has streamed to completion.
  if (runtime.phase === 'complete' && quizScore === null && sessionCategory !== null) {
    return (
      <SentenceReadingModeQuiz
  ...
  return orientation === 'vertical' ? (
    <SentenceReadingModeVerticalCanvas {...canvasProps} />
  ) : (
    <SentenceReadingModeCanvas {...canvasProps} />
  )
}
