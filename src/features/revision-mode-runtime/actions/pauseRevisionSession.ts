...
// Revision Mode™ Sprint ALS-17. A thin wrapper naming which real LSE-2
// decision this action applies, mirroring Memory Mode™'s own
// `pauseMemorySession.ts`.
export async function pauseRevisionSession(input: unknown): Promise<ModeSessionActionResult> {
  const parsed = SessionIdSchema.safeParse(input)
  if (!parsed.success) return { success: false, error: 'Invalid revision session request.' }

  return runRevisionSessionDecision(parsed.data, (runtime) => pauseRuntime(runtime))
}
