...
type SensoryHologramBuilderCompleteScreenProps = {
  goalTitle: string
  elapsedMs: number
...
// are hard WPM-shaped and don't describe a guided sensory visualization
...
// shared component.
export function SensoryHologramBuilderCompleteScreen({
  goalTitle,
  elapsedMs,
  immersionScorePercent,
  streak,
  bestScorePercentAllTime,
  bestStreakAllTime,
  onPlayAgain,
  backHref = '/labs/quantum-speed-reading',
}: SensoryHologramBuilderCompleteScreenProps): React.JSX.Element {
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center gap-10 px-6 py-16 text-center">
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Hologram Complete</h1>
        <p className="mt-2 text-sm text-muted-foreground">{goalTitle}</p>
      </div>

      <div className="grid w-full grid-cols-2 gap-4">
        <ReadingStatTile variant="card" label="Sensory Immersion Score" value={`${immersionScorePercent}%`} />
        <ReadingStatTile variant="card" label="Session Streak" value={String(streak)} />
