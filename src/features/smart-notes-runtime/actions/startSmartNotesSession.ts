...
// Smart Notes™ Sprint-1. Learning Mode Registration + Learning Mode
// Loader + Session Creation, composed exactly like Quantum Speed
// Reading™'s own `startReadingSession.ts` and Memory Mode™'s own
...
// LSE-4's own `startModeRuntime`. The resulting `AdaptiveRuntimeState` is
// never persisted directly — only its real, derived `SessionSnapshot`
// (LSE-3) is, via the same Shared Learning Runtime persistence adapter
// QSR and Memory Mode use.
const StartSmartNotesSessionInputSchema = z.object({
  documentId: z.string().uuid(),
  chunkStrategy: ChunkStrategySchema.default('sequential'),
})

export async function startSmartNotesSession(input: unknown): Promise<ModeSessionActionResult> {
  const parsed = StartSmartNotesSessionInputSchema.safeParse(input)
  ...
  const result = startModeRuntime(registry, 'smart-notes', ulo, { learnerId: user.id, chunkStrategy: parsed.data.chunkStrategy })
  ...
  try {
    ...
    return { success: false, error: 'Failed to persist the new smart notes session.' }
  }
