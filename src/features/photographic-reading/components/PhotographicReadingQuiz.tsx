...
import type { FlashRecallSprintQuizQuestion } from '../photographicReadingDataset'
...
type PhotographicReadingQuizProps = {
...
function scoreMessage(score: number, total: number): string {
  if (score === total) return 'Perfect recall — your eyes captured every key idea across the whole spatial field.'
  if (score >= Math.ceil(total / 2)) return 'Solid comprehension — most of it stuck even while jumping between corners.'
  return 'Worth another pass — a few key ideas slipped by while the eyes were relocating.'
}

// Post-session MCQ comprehension quiz — appears only after every spatial
// cluster has finished flashing, with zero mid-exercise interruptions
// beforehand. Own-copy of SubvocalizationDestroyerQuiz.tsx's own state
// machine (question -> instant feedback -> next, repeated, then a results
// view) — deliberately its own small component rather than a new phase
// inside the locked useReadingRuntime.ts, which stays untouched and shared
// by every other exercise.
export function PhotographicReadingQuiz({ questions, categoryLabel, onComplete, onExit }: PhotographicReadingQuizProps): React.JSX.Element {
