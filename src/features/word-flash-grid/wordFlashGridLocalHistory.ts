// Best-stats store for Word Flash Grid™ — a small, narrowly scoped
// parallel to every other exercise's own local-history file in this
// project (numberFlashGridLocalHistory.ts, dotMemoryGridLocalHistory.ts),
...
// NumberFlashGridBestStats/DotMemoryGridBestStats exactly
...
export type WordFlashGridBestStats = {
  bestScorePercent: number
  bestStreak: number
}

const EMPTY_BEST: WordFlashGridBestStats = { bestScorePercent: 0, bestStreak: 0 }

export function loadBestWordFlashGridStats(storageKey: string): WordFlashGridBestStats {
...
export function recordBestWordFlashGridStats(storageKey: string, session: WordFlashGridBestStats): WordFlashGridBestStats {
  const previous = loadBestWordFlashGridStats(storageKey)
  const next: WordFlashGridBestStats = {
