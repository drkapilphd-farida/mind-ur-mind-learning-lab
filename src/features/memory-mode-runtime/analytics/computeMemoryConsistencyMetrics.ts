...
// Consistency Metrics (item 5). Pure (the real clock is injected, never
// read directly, matching LSE-2's own `{ now }` convention). Buckets real
// sessions by real UTC calendar day (never local time, so results stay
// deterministic regardless of server timezone) — `activeDays` is the real
// count of distinct days with at least one session, `longestStreakDays`
// is the longest real run of consecutive days, `currentStreakDays` counts
// backward from today only if today itself had real activity, and
// `averageSessionsPerActiveDay` is a real, honest ratio.
...
export function computeMemoryConsistencyMetrics(snapshots: readonly SessionSnapshot[], now: () => Date = () => new Date()): MemoryConsistencyMetrics {
