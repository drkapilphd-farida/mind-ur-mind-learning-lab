...
type VerticalFlashRecallSettingsProps = {
  targetWpm: number
...
export function VerticalFlashRecallSettings({
  targetWpm,
  onSelectTargetWpm,
  onStart,
  categoryLabel,
}: VerticalFlashRecallSettingsProps): React.JSX.Element {
  ...
  return (
    <div className={`relative mx-auto flex ${isEmbedded ? 'h-full' : 'min-h-[100dvh]'} max-w-md flex-col items-center justify-center gap-8 px-6 py-16 text-center`}>
      ...
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Vertical Flash Recall &amp; Retention Sprint™</h1>
        <p className="mt-3 text-sm text-muted-foreground">
          Words flash one at a time down a fixed vertical column at your target pace, training rapid vertical eye-span — then 3 quick questions check what you retained.
        </p>
        {/* Deliberately rendered as null on both the server and the
            client's first paint (only ever set from a useEffect in the
            Experience orchestrator, never a lazy state initializer) —
            see verticalFlashRecallDataset.ts's pickSessionCategory doc
