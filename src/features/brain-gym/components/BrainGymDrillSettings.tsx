...
type BrainGymDrillSettingsProps = {
  config: BrainGymDrillConfig
  onStart: () => void
}

// Same visual convention as EspZenerTelepathySettings.tsx/
// SchulteGridDrillSettings.tsx — no per-attempt configuration exists for
// any of the 4 Brain Gym drills, so this is purely the instructions/
// intro gate every advanced exercise has before Start.
export function BrainGymDrillSettings({ config, onStart }: BrainGymDrillSettingsProps): React.JSX.Element {
  return (
    <div className="relative mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center gap-8 px-6 py-16 text-center">
      ...
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">{config.title}</h1>
        <p className="mt-3 text-sm text-muted-foreground">{config.instructions}</p>
      </div>
