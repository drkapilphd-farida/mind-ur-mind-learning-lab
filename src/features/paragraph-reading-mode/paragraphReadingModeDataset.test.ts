...
import {
  PARAGRAPH_READING_MODE_CATEGORIES,
  TOTAL_PARAGRAPH_READING_MODE_CATEGORIES,
  buildUnitsForCategory,
  pickSessionCategory,
} from './paragraphReadingModeDataset'

describe('PARAGRAPH_READING_MODE_CATEGORIES', () => {
  it('has between 20 and 24 categories per spec', () => {
    expect(TOTAL_PARAGRAPH_READING_MODE_CATEGORIES).toBeGreaterThanOrEqual(20)
    expect(TOTAL_PARAGRAPH_READING_MODE_CATEGORIES).toBeLessThanOrEqual(24)
  })

  it('gives every category a unique id', () => {
    const ids = new Set(PARAGRAPH_READING_MODE_CATEGORIES.map((category) => category.id))
    expect(ids.size).toBe(PARAGRAPH_READING_MODE_CATEGORIES.length)
  })

  it('gives every category a unique label', () => {
    const labels = new Set(PARAGRAPH_READING_MODE_CATEGORIES.map((category) => category.label))
    expect(labels.size).toBe(PARAGRAPH_READING_MODE_CATEGORIES.length)
  })

  it('never repeats the exact same passage text across two different categories', () => {
    const allTexts = PARAGRAPH_READING_MODE_CATEGORIES.map((category) => category.text)
    const unique = new Set(allTexts)
    expect(unique.size).toBe(allTexts.length)
  })

  // A true full-length passage — at 250 WPM, 280 words takes ~67s and 340
  // words takes ~82s, a genuine multi-minute-scale continuous read at a
  // real target pace, meaningfully longer than the sibling reading modes'
  // own 250-320 word band, matching this mode's "full paragraph" premise.
  it('gives every category a 250-350 word full-length passage', () => {
    for (const category of PARAGRAPH_READING_MODE_CATEGORIES) {
  ...
  it('gives every question a globally unique id', () => {
    const ids = PARAGRAPH_READING_MODE_CATEGORIES.flatMap((category) => category.questions.map((question) => question.id))
...
describe('buildUnitsForCategory', () => {
  it('produces one unit per real word, with unique ids, for every category', () => {
    for (const category of PARAGRAPH_READING_MODE_CATEGORIES) {
...
describe('pickSessionCategory', () => {
  ...
  it('always returns a real category from the list', () => {
    ...
    expect(PARAGRAPH_READING_MODE_CATEGORIES.some((category) => category.id === picked.id)).toBe(true)
  })

  it('persists the picked category id to its own storage key', () => {
    const picked = pickSessionCategory()
    expect(store['qsr-paragraph-reading-mode-last-category']).toBe(picked.id)
  })
