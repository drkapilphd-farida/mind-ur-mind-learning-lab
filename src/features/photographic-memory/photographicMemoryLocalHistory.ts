// Best-stats store for Photographic Memory™ — a small, narrowly scoped
// parallel to every other exercise's own local-history file in this
// project, not shared with any of them. Same SSR-safe localStorage
// convention and "0 means no record yet" sentinel every other
// local-history file in this project already establishes.
export type PhotographicMemoryBestStats = {
  bestScorePercent: number
  bestStreak: number
}

const EMPTY_BEST: PhotographicMemoryBestStats = { bestScorePercent: 0, bestStreak: 0 }

export function loadBestPhotographicMemoryStats(storageKey: string): PhotographicMemoryBestStats {
...
export function recordBestPhotographicMemoryStats(
  storageKey: string,
  session: PhotographicMemoryBestStats,
): PhotographicMemoryBestStats {
  const previous = loadBestPhotographicMemoryStats(storageKey)
  const next: PhotographicMemoryBestStats = {
