...
type VerticalWordReadingSettingsProps = {
  targetWpm: number
...
export function VerticalWordReadingSettings({
  targetWpm,
  onSelectTargetWpm,
  onStart,
  categoryLabel,
}: VerticalWordReadingSettingsProps): React.JSX.Element {
  ...
  return (
    <div className={`relative mx-auto flex ${isEmbedded ? 'h-full' : 'min-h-[100dvh]'} max-w-md flex-col items-center justify-center gap-8 px-6 py-16 text-center`}>
      ...
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Vertical Word Reading Engine™</h1>
        <p className="mt-3 text-sm text-muted-foreground">
          Real vocabulary glides smoothly down a central focus window at your target pace — a fresh topic every session.
        </p>
        {/* Deliberately rendered as null on both the server and the
            ...
            see verticalWordReadingDataset.ts's pickSessionCategory doc
