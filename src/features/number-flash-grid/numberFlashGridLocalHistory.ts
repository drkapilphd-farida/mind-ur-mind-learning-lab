// Best-stats store for Number Flash Grid™ — a small, narrowly scoped
// parallel to every other exercise's own local-history file in this
// project (dotMemoryGridLocalHistory.ts, photographicMemoryLocalHistory.ts),
...
// PhotographicMemoryBestStats/DotMemoryGridBestStats exactly
// (bestScorePercent/bestStreak) so RightBrainHubModeCard.tsx's
// BEST_STATS_LOADERS map can reference this loader directly, with no
// normalization wrapper needed.
export type NumberFlashGridBestStats = {
  bestScorePercent: number
  bestStreak: number
}

const EMPTY_BEST: NumberFlashGridBestStats = { bestScorePercent: 0, bestStreak: 0 }

export function loadBestNumberFlashGridStats(storageKey: string): NumberFlashGridBestStats {
...
export function recordBestNumberFlashGridStats(storageKey: string, session: NumberFlashGridBestStats): NumberFlashGridBestStats {
  const previous = loadBestNumberFlashGridStats(storageKey)
  const next: NumberFlashGridBestStats = {
