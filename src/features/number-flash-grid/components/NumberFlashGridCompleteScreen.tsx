...
import { NUMBER_FLASH_GRID_ROUNDS_PER_SESSION, computeAccuracyPercent } from '../numberFlashGridEngine'

type NumberFlashGridCompleteScreenProps = {
  elapsedMs: number
  totalCorrect: number
  totalDigits: number
  bestStreak: number
...
// are hard WPM-shaped and don't describe a numerical flash-then-recall
// game honestly. Same visual language/classes as
// DotMemoryGridCompleteScreen.tsx / EspZenerTelepathyCompleteScreen.tsx,
// without modifying or forking either locked component.
export function NumberFlashGridCompleteScreen({
  elapsedMs,
  totalCorrect,
  totalDigits,
  bestStreak,
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center gap-10 px-6 py-16 text-center">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Nice numerical recall.</p>
      </div>

      <div className="grid w-full grid-cols-2 gap-4">
        <ReadingStatTile variant="card" label="Rounds Played" value={String(NUMBER_FLASH_GRID_ROUNDS_PER_SESSION)} />
        <ReadingStatTile variant="card" label="Correct Placements" value={`${totalCorrect} / ${totalDigits}`} />
