...
export async function resumeIntelligentReadingSession(input: unknown): Promise<IntelligentReadingSessionResult> {
  ...
  return persistAndRespond(contextResult.context, parsed.data.documentId, parsed.data.chapterOrder, { ...resolved, paused: false })
}
