...
type DynamicDotIntroProps = {
  onStart: () => void
}

export function DynamicDotIntro({ onStart }: DynamicDotIntroProps): React.JSX.Element {
    ...
    <div className="mx-auto flex w-full max-w-md flex-col items-center gap-8 text-center">
      ...
      >
        <div className="flex size-14 items-center justify-center rounded-full bg-primary/[0.12] text-primary">
          <Move className="size-7" aria-hidden="true" />
        </div>
      </div>

      <div>
        <p className="text-xs font-medium tracking-widest text-muted-foreground uppercase">Visual Fixation Engine™</p>
        <h1 className="mt-2 font-heading text-3xl font-bold tracking-tight text-foreground">Dynamic Dot™</h1>
        <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-muted-foreground">
          Track the moving dot with your eyes — without moving your head.
        </p>
      </div>
