...
// ResidentialStickyBar.tsx/MentoringStickyBar.tsx.
const SCROLL_REVEAL_THRESHOLD_PX = 560;

export default function CourseStickyBar(): React.JSX.Element {
  const { t } = useLanguage();
    ...
    >
      <div className="mx-auto flex max-w-content items-center justify-between gap-4 px-6 py-3.5 pb-[max(0.875rem,env(safe-area-inset-bottom))] sm:px-8">
        <div className="min-w-0">
          <p className="truncate text-[13.5px] font-semibold text-ink">{course.stickyBar.text}</p>
          <p className="font-mono text-[11px] uppercase tracking-[0.06em] text-ink-faint">{course.stickyBar.price}</p>
        </div>
        <a
          href={CLASSPLUS_OVERTHINKING_COURSE_LINK}
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => trackGaEvent("classplus_click", { location: "course_sticky_bar" })}
