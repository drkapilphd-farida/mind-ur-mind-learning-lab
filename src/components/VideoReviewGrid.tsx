...
export default function VideoReviewGrid({ videos, className = "" }: VideoReviewGridProps): React.JSX.Element {
      ...
      <div className={`grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 ${className}`}>
        {videos.map((video) => (
          <button
            ...
            className="group relative aspect-video overflow-hidden rounded-sm border border-line-strong bg-panel2"
          >
            {video.thumbnailSrc !== undefined ? (
              <Image
                src={video.thumbnailSrc}
                alt="Real student video review"
                fill
      ...
      {openVideoId !== null && (
        ...
        >
          <div className="relative w-full max-w-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="aspect-video w-full overflow-hidden rounded-sm border border-line-strong bg-void">
              <iframe
